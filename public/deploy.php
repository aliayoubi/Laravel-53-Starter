...
// FTP Credentials
$ftp_host = 'xxxxxxxxxxxxxxxxxx';
$ftp_username = 'xxxxxxxxxxxxxxxxxx';
...
//$ftp_path = dirname(__DIR__) . DIRECTORY_SEPARATOR;
$ftp_path = '/public_html/dev/';
...
foreach ($files as $file) {
    ...
    if ($path === 'app.php' || $path === 'config/app.php') {
        continue;
    }
    if ($path === 'database.php' || $path === 'config/database.php') {
        continue;
    }
    ...
    if ($file->type == "removed") {
        ...
        // change file permission to 644/writable
        if (!is_dir($ftp_path . $path)) {
            @chmod($ftp_path . $path, 0644);
        }
